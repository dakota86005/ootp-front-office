/**
 * One definition for every stat, rating and derived value the app puts on screen, so a column means the same thing on
 * every page and in both clients: the React app's `src/glossary.ts` reads it, and the Mac app is served it through
 * `GET /api/v2/catalog` (SWIFTUI_REBUILD.md section 4). Pure data and one pure function: it loads in both the server
 * and the browser bundle.
 *
 * Stat lines carry their own descriptions in `statCatalog.ts`; those are folded in and overridden here only where a page
 * needs a different emphasis. Everything else (roster status, scouting, contract and schedule vocabulary) is below.
 */
import { BATTING_STATS, PITCHING_STATS } from './statCatalog.js';

export const GLOSSARY_EXTRA: Readonly<Record<string, string>> = {
  // ── Identity and roster ────────────────────────────────────────────────
...
function fromStats(): Record<string, string> {
...
/** Every term and its definition: the stat descriptions, then the vocabulary above over them. */
export function glossaryTable(): Record<string, string> {
  return { ...fromStats(), ...GLOSSARY_EXTRA };
}

/**
 * Terms only the React app's pages use, which the Mac app is not served: "Recommendation" names the Contracts page's
 * verdict column, which the Mac app will not have (D-001), and "Pot Δ" is the development page's column for a change in
 * OOTP's ceiling figure, a name the plain-language list keeps off the Mac app's face.
 */
export const REACT_ONLY_TERMS: readonly string[] = ['Recommendation', 'Pot Δ'];

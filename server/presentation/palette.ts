/**
 * Derives a club's palette from its colours as OOTP exports them, for both clients: the React app applies it as CSS
 * variables (`src/theme.ts`), and the Mac app is served it as hex tokens per club through `GET /api/v2/catalog`
 * (`clubPalette`). `npm run check:theme` checks every club's palette for contrast, reading this module.
 *
 * The design constraint: the app is dense with tables, so the page must stay readable whatever a team's colours are.
 * Team identity comes through as a tint on the surfaces plus a vivid accent, rather than letting (say) a white or
 * yellow primary wash out the page. Pure: no imports, so the browser bundle loads it too.
 */
...
/** A CSS colour this module writes (`hsl(h, s%, l%)` or `#rrggbb`) as `#rrggbb`. */
export function toHex(css: string): string {
  const hex = /^#?([0-9a-f]{6})$/i.exec(css.trim());
  if (hex) return `#${hex[1].toLowerCase()}`;
  const m = /^hsl\((-?[\d.]+),\s*([\d.]+)%,\s*([\d.]+)%\)$/.exec(css.trim());
  if (!m) throw new Error(`Not a colour this palette writes: ${css}`);
...
/** A club's palette in one mode, as the Mac app is served it: every token as `#rrggbb`. */
export interface ClubPalette {
  /** The club's plate (its primary colour, adjusted until its text reads) and the text on it. */
  team: string;
  teamText: string;
  background: string;
...
/** The palette `derivePalette` makes, as hex tokens with names. */
export function clubPalette(colors: TeamColors | null, mode: ThemeMode): ClubPalette {
  const p = derivePalette(colors, mode);
  const hex = (key: string): string => toHex(p[key]);
  return {
    team: hex('--team'),
    teamText: hex('--team-fg'),
    background: hex('--bg'),
    backgroundDeep: hex('--bg-deep'),
    backgroundGlow: hex('--bg-glow'),
    panel: hex('--panel'),
    panelRaised: hex('--panel-raised'),
    border: hex('--border'),
    text: hex('--text'),
    muted: hex('--muted'),
    accent: hex('--accent'),
    accentText: hex('--accent-ink'),
    good: hex('--good'),
    bad: hex('--bad'),
  };
}
